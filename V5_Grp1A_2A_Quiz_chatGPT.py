import streamlit as st
import random
import time
import base64

st.set_page_config(page_title="Chemistry Quiz", page_icon="🔬")

st.title("🔬 Chemistry Quiz: Group 1A & 2A Elements")

# -----------------------------
# ELEMENT DATA
# -----------------------------
elements = {
    "H": "hydrogen",
    "Li": "lithium",
    "Na": "sodium",
    "K": "potassium",
    "Rb": "rubidium",
    "Cs": "cesium",
    "Fr": "francium",
    "Be": "beryllium",
    "Mg": "magnesium",
    "Ca": "calcium",
    "Sr": "strontium",
    "Ba": "barium",
    "Ra": "radium"
}

# -----------------------------
# SOUND LOADING
# -----------------------------
def load_sound(file_path):
    with open(file_path, "rb") as f:
        data = f.read()
    return base64.b64encode(data).decode()

correct_sound = load_sound("correct.mp3")
incorrect_sound = load_sound("incorrect.mp3")
timeout_sound = load_sound("timeout.mp3")

def play_sound(sound_b64):
    sound_html = f"""
        <audio autoplay>
            <source src="data:audio/mp3;base64,{sound_b64}" type="audio/mp3">
        </audio>
    """
    st.markdown(sound_html, unsafe_allow_html=True)

# -----------------------------
# SESSION STATE INITIALIZATION
# -----------------------------
if "symbol" not in st.session_state:
    st.session_state.symbol = random.choice(list(elements.keys()))

if "score" not in st.session_state:
    st.session_state.score = 0

if "total" not in st.session_state:
    st.session_state.total = 0

if "start_time" not in st.session_state:
    st.session_state.start_time = time.time()

if "feedback" not in st.session_state:
    st.session_state.feedback = ""

if "multiple_choice" not in st.session_state:
    st.session_state.multiple_choice = False

# -----------------------------
# SETTINGS SIDEBAR
# -----------------------------
st.sidebar.header("Settings")
st.session_state.multiple_choice = st.sidebar.checkbox("Enable multiple‑choice mode")
timer_limit = st.sidebar.slider("Seconds per question", 5, 30, 12)

# -----------------------------
# TIMER
# -----------------------------
elapsed = int(time.time() - st.session_state.start_time)
remaining = timer_limit - elapsed
st.progress(min(max(remaining / timer_limit, 0), 1))

if remaining <= 0:
    st.session_state.feedback = (
        f"⏱️ Time's up! The correct answer was **{elements[st.session_state.symbol]}**."
    )
    play_sound(timeout_sound)
    st.session_state.total += 1
    st.session_state.symbol = random.choice(list(elements.keys()))
    st.session_state.start_time = time.time()

# -----------------------------
# QUESTION DISPLAY
# -----------------------------
st.subheader("Identify the element name (lowercase)")
st.markdown(f"### Symbol: **{st.session_state.symbol}**")

# -----------------------------
# MULTIPLE CHOICE MODE
# -----------------------------
if st.session_state.multiple_choice:
    correct = elements[st.session_state.symbol]
    choices = [correct]

    while len(choices) < 4:
        option = random.choice(list(elements.values()))
        if option not in choices:
            choices.append(option)

    random.shuffle(choices)
    answer = st.radio("Choose the correct element:", choices)

else:
    answer = st.text_input("Type your answer:")

# -----------------------------
# SUBMIT BUTTON
# -----------------------------
if st.button("Submit"):
    correct = elements[st.session_state.symbol]
    st.session_state.total += 1

    if answer.strip().lower() == correct:
        st.session_state.score += 1
        st.session_state.feedback = f"✅ Correct! **{correct}**."
        play_sound(correct_sound)
    else:
        st.session_state.feedback = f"❌ Incorrect. The correct answer is **{correct}**."
        play_sound(incorrect_sound)

    st.session_state.symbol = random.choice(list(elements.keys()))
    st.session_state.start_time = time.time()

# -----------------------------
# FEEDBACK + SCOREBOARD
# -----------------------------
st.write(st.session_state.feedback)

st.markdown("---")
st.subheader("📊 Progress")
st.write(f"Score: **{st.session_state.score} / {st.session_state.total}**")

if st.session_state.total > 0:
    accuracy = (st.session_state.score / st.session_state.total) * 100
    st.write(f"Accuracy: **{accuracy:.1f}%**")
